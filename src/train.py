...
def train_step(
    ...
    # ! Added a new parameter for the output_time_frame.
    motion_pred = torch.matmul(
        idct_m[:, : config.motion.pi3d_input_length, : config.motion.pi3d_input_length],
        motion_pred,
    )

    # ! We always predict the same size as the input but when dealing with loss we cut to our actual output size.
    # We do not need to redo previous trainings since we used 10 and 10.
...
if __name__ == "__main__":
    ...
    # add number of parameters to wandb
    wandb.watch(model)
    ...
    while (nb_iter + 1) < config.cos_lr_total_iters:
        for pi3d_motion_input, pi3d_motion_target in dataloader:
            ...
            # print(nb_iter)
            avg_loss += loss
            # every 100 iterations, print the loss and save the model
            if (nb_iter + 1) % 100 == 0:
                avg_loss /= 100
                print(
            ...
            if (nb_iter + 1) % config.save_every == 0:
                ...
                model.eval()
                acc_tmp = test(eval_config, model, eval_dataloader, device, dct_m, idct_m)
                # log mpjpe to wandb
                mpjpe = acc_tmp[0]
                ...
                # print (mpjpe_02_sec, mpjpe_04_sec, mpjpe_08_sec, mpjpe_1_sec)
                print(
